...
PROVIDER_CATALOG: tuple[ProviderSpec, ...] = (
    ...
    ProviderSpec(
        code='soliscloud_v2',
        name='SolisCloud API V2/V3',
        provider='solis',
        auth_mode='hmac_api_key',
        base_url='https://www.soliscloud.com:13333',
        healthcheck_endpoint='/v2/api/inverterDetail',
        sync_endpoint='/v2/api/inverterDetail',
        required_fields=('inverter_sn', 'api_key', 'api_secret'),
        optional_fields=('station_id',),
        category='inverter_cloud',
        docs_url='https://doc.soliscloud.com/en/20.API%20documentation/01.SolisCloud%20Platform%20API%20Document.html',
        notes_ar='مخطط تكامل SolisCloud. يحتاج تفعيل API من حساب Solis وتوقيع HMAC قبل المزامنة الفعلية.',
        notes_en='SolisCloud integration blueprint. Requires API access and request signing before live sync.',
        mapping_schema={'solar_power': 'data.pac', 'battery_soc': 'data.batteryCapacitySoc', 'daily_production': 'data.eToday', 'total_production': 'data.eTotal'},
    ),
    ProviderSpec(
        code='opendtu_local',
        name='OpenDTU Local Web API',
        provider='opendtu',
        auth_mode='local_http_optional_auth',
        base_url='http://192.168.1.60',
        healthcheck_endpoint='/api/livedata/status',
        sync_endpoint='/api/livedata/status',
        required_fields=('local_base_url',),
        optional_fields=('username', 'password'),
        category='local_inverter',
        docs_url='https://www.opendtu.solar/firmware/web_api/',
        notes_ar='تكامل محلي لمراقبة Hoymiles عبر OpenDTU داخل الشبكة. لا يعتمد على السحابة.',
        notes_en='Local Hoymiles/OpenDTU monitoring over LAN without cloud dependency.',
        mapping_schema={'solar_power': 'total.Power.v', 'daily_production': 'total.YieldDay.v', 'total_production': 'total.YieldTotal.v'},
    ),
    ProviderSpec(
        code='home_assistant_energy',
        name='Home Assistant Energy API',
        provider='home_assistant',
        auth_mode='long_lived_access_token',
        base_url='http://homeassistant.local:8123',
        healthcheck_endpoint='/api/',
        sync_endpoint='/api/states/{entity_id}',
        required_fields=('local_base_url', 'access_token', 'entity_id'),
        optional_fields=('solar_entity_id', 'battery_entity_id', 'grid_entity_id'),
        category='energy_gateway',
        docs_url='https://developers.home-assistant.io/docs/api/rest/',
        notes_ar='بوابة تكامل عامة لأي جهاز مربوط في Home Assistant. مناسب كبوابة عالمية للأجهزة المحلية.',
        notes_en='Generic gateway for any Home Assistant-connected energy device.',
        mapping_schema={'solar_power': 'state', 'raw_data': 'attributes'},
    ),
    ProviderSpec(
        code='modbus_tcp_energy',
        name='Generic Modbus TCP Energy Device',
        provider='modbus',
        auth_mode='local_modbus_tcp',
        base_url=None,
        healthcheck_endpoint=None,
        sync_endpoint=None,
        required_fields=('host', 'port', 'unit_id', 'register_map'),
        category='local_protocol',
        docs_url='https://www.home-assistant.io/integrations/modbus/',
        notes_ar='مخطط بروتوكول عام للقراءة من أجهزة Modbus TCP. يحتاج adapter خاص في مرحلة التشغيل الفعلي.',
        notes_en='Generic Modbus TCP blueprint. Live polling requires a dedicated Modbus adapter.',
        mapping_schema={'register_map': 'settings.register_map'},
    ),
    ProviderSpec(
        code='mqtt_energy_gateway',
        name='Generic MQTT Energy Gateway',
        provider='mqtt',
        auth_mode='mqtt_broker',
        base_url=None,
        healthcheck_endpoint=None,
        sync_endpoint=None,
        required_fields=('broker_host', 'topic_map'),
        optional_fields=('broker_port', 'username', 'password'),
        category='energy_gateway',
        docs_url='https://mqtt.org/',
        notes_ar='مخطط Gateway للأجهزة التي تنشر القياسات عبر MQTT مثل عدادات الطاقة وESPHome وOpenDTU.',
        notes_en='Gateway blueprint for energy devices publishing telemetry through MQTT.',
        mapping_schema={'topic_map': 'settings.topic_map'},
    ),
    ProviderSpec(
        code='kostal_plenticore_local',
        name='KOSTAL PLENTICORE Local API',
        provider='kostal',
        auth_mode='local_http',
        base_url='http://192.168.1.70',
        healthcheck_endpoint='/api/v1/status',
        sync_endpoint='/api/v1/processdata/devices:local',
        required_fields=('local_base_url',),
        optional_fields=('username', 'password'),
        category='local_inverter',
        docs_url='https://www.kostal-solar.com/',
        notes_ar='Blueprint محلي لأجهزة KOSTAL. قد تختلف المسارات حسب firmware.',
        notes_en='Local KOSTAL blueprint. Endpoints may vary by firmware.',
        mapping_schema={'raw_data': 'raw'},
    ),
    ProviderSpec(
        code='apsystems_ema',
        name='APsystems EMA / ECU',
        provider='apsystems',
        auth_mode='cloud_or_local_ecu',
        base_url=None,
        healthcheck_endpoint=None,
        sync_endpoint=None,
        required_fields=('base_url', 'system_id'),
        optional_fields=('api_key', 'username', 'password'),
        category='microinverter_cloud',
        docs_url='https://apsystems.com/',
        notes_ar='Blueprint لمراقبة APsystems عبر EMA أو ECU حسب التوفر.',
        notes_en='APsystems EMA/ECU monitoring blueprint depending on API availability.',
        mapping_schema={'raw_data': 'raw'},
    ),
    ProviderSpec(
        code='tigo_energy_intelligence',
        name='Tigo Energy Intelligence',
        provider='tigo',
        auth_mode='cloud_api',
        base_url=None,
        healthcheck_endpoint=None,
        sync_endpoint=None,
        required_fields=('system_id', 'api_key'),
        category='optimizer_cloud',
        docs_url='https://www.tigoenergy.com/',
        notes_ar='Blueprint لبيانات المحسنات والمراقبة من Tigo عند توفر API تجاري.',
        notes_en='Blueprint for Tigo optimizer and monitoring data when commercial API access is available.',
        mapping_schema={'raw_data': 'raw'},
    ),
    ProviderSpec(
        code='growatt_v1',
...
PROVIDER_ALIASES = {
    ...
    'growatt': 'growatt_v1',
    'solis': 'soliscloud_v2',
    'soliscloud': 'soliscloud_v2',
    'opendtu': 'opendtu_local',
    'hoymiles': 'opendtu_local',
    'homeassistant': 'home_assistant_energy',
    'ha': 'home_assistant_energy',
    'modbus': 'modbus_tcp_energy',
    'mqtt': 'mqtt_energy_gateway',
    'kostal': 'kostal_plenticore_local',
    'apsystems': 'apsystems_ema',
    'tigo': 'tigo_energy_intelligence',
    'shelly': 'shelly_gen2',
}
...
def provider_category_label(category: str | None, lang: str = 'ar') -> str:
    ...
    labels = {
        ...
        'energy_gateway': {'ar': 'بوابة طاقة', 'en': 'Energy gateway'},
        'local_protocol': {'ar': 'بروتوكول محلي', 'en': 'Local protocol'},
        'microinverter_cloud': {'ar': 'مايكرو انفيرتر', 'en': 'Microinverter'},
        'optimizer_cloud': {'ar': 'محسنات شمسية', 'en': 'Optimizer platform'},
